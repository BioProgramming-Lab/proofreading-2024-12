...
def RD_rxn(c_tuple, t, rxn_coeffs, production_rate):
    ...
    # Unpack
    c_A, c_B, c_C, c_R, c_AC, c_BC, c_AR, c_BR, _ = c_tuple
    ...
    ar_rate = rxn_coeffs.k_AR * c_A * c_R - \
        (rxn_coeffs.r_AR + rxn_coeffs.gamma) * c_AR - rxn_coeffs.deg * c_AR

    # B-Receptor reaction rate (assume constant turnover)
    br_rate = rxn_coeffs.k_BR * c_B * c_R - \
        (rxn_coeffs.r_BR + rxn_coeffs.gamma) * c_BR - rxn_coeffs.deg * c_BR

    energy_rate = j_A + j_B + j_C \
                    + j_self_activation_ar_on_a * hill_a(c_AR) \
                    + j_self_activation_ac_on_ac * hill_a(c_AC) \
                    + j_mutual_inhibition_bc_on_ac * hill_r(c_BC) \
                    + j_ac_rp * hill_ar(c_AC, c_BC)\
    ...
    return (A_rate, B_rate, C_rate, R_rate, ac_rate, bc_rate, ar_rate, br_rate, energy_rate)
